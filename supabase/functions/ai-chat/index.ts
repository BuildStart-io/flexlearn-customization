...
${faqContext || "No FAQs configured"}`;
    ...
    // Extract Customer Type & Lead Stage tags
    ...
    // Upsert / Update Lead with classification metadata
    if (customerType || leadStage) {
      try {
        ...
        console.log(`[ai-chat] Updated lead for ${phoneNumber}: type=${customerType}, stage=${leadStage}`);
      } catch (leadErr) {
        console.warn("[ai-chat] Could not update lead classification:", leadErr);
      }
    }
