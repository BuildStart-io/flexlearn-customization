...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const row of settingsRows || []) {
      ...
      const renewalEnabled = renewalCfg.enabled ?? true;
              ...
              if (!priorStage) {
                try {
                  ...
                  if (res.ok) {
                    ...
                    console.log(`[send-followups-flexlearn-customization] Renewal reminder (${stage.name}) sent to ${phone}`);
                  }
                } catch (stageErr) {
